    ...
    for param, value in order.items():
        ...
        for item in created_order['detail']:
            if item['ingredient'] is not None:
                ingredients_in_detail.append(item['ingredient']['_id'])
            if item['beverage'] is not None:
                beverages_in_detail.append(item['beverage']['_id'])
...
def test_calculate_order_price_returns_correct_price_with_database_data(app, ingredients, beverages, size, client_data):
    ...
    test_price = round(created_size['price'] + sum(ingredient['price'] for ingredient in created_ingredients) + sum(beverage['price'] for beverage in created_beverages), 2)
