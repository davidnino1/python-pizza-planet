...
@pytest.fixture
def create_beverage(client, beverage_url) -> dict:
    response = client.post(beverage_url, json=beverage_mock())
    return response


@pytest.fixture
def create_beverages(client, beverage_url) -> list:
    beverages = []
    for _ in range(10):
        new_beverage = client.post(beverage_url, json=beverage_mock())
